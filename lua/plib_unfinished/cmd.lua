...
-- Defualt parsers
local function playercomplete(cmd, arg, step)
	local ret = {}
...
function cmd.Run(caller, name, str_args)
	...
	local succ, msg = (hook.Call('cmd.CanRunCommand', nil, caller, cmd, msg, args) or cmd:CanRun(caller, args) or cmd.Callback(caller, args))
	...
	if (not concommands[name]) then
		...
		if (SERVER) then
			...
			concommand.Add('_' .. name, function(pl, cmd, args, str)
				LocalPlayer():ConCommand(name .. ' ' .. str)
